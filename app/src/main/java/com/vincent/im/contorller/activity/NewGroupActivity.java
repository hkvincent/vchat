...
/**
 * Created by Administrator on 2018/4/5 20:12
 */
    ...
    private void createGroup(final String[] memberses) {
        ...
        Model.getInstance().getGlobalThreadPool().execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    EMClient.getInstance().groupManager().createGroup(groupName, groupDesc, memberses, "apply to join group", options);

                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(NewGroupActivity.this, "create group successfully", Toast.LENGTH_SHORT).show();
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(NewGroupActivity.this, "create group failed", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            }
